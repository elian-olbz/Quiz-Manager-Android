...
public class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (drawHorizontalEdges) {
            canvas.scale(SHADOW_BOTTOM_SCALE / shadowScaleHorizontal, SHADOW_BOTTOM_SCALE);
            canvas.drawRect(0.0f, edgeShadowTop, this.contentBounds.width() - (2.0f * shadowOffset), this.shadowSize + (-this.cornerRadius), this.edgeShadowPaint);
        }
    ...
    public float getMinWidth() {
        return (this.rawMaxShadowSize * 2.0f) + (2.0f * Math.max(this.rawMaxShadowSize, this.cornerRadius + (this.rawMaxShadowSize / 2.0f)));
    }

    public float getMinHeight() {
        return (this.rawMaxShadowSize * SHADOW_MULTIPLIER * 2.0f) + (2.0f * Math.max(this.rawMaxShadowSize, this.cornerRadius + ((this.rawMaxShadowSize * SHADOW_MULTIPLIER) / 2.0f)));
    }
}
